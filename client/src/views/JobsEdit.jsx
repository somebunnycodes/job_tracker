...
const JobsNew = () => {
  ...
  const updateJob = () => {
    axios.put(`${REACT_APP_API_URI}/api/jobs/${job_id}`, { ...job, user_id: localStorage.getItem('userId') }, { withCredentials: true })
      .then(res => {
        setValidationErrors({})
        navigate('/jobs')
      })
  ...
  return (
    <>
      <HeaderUser />
      <JobForm job={job} setJob={setJob} submitText='Save' onSubmitProp={updateJob} validationErrors={validationErrors} />
    </>
  )
}
